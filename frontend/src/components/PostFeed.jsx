...
function PostFeed({ title, mine = false, emptyMessage }) {
  const { username: loggedInUsername } = useAuth()
  const currentUsername = loggedInUsername ?? 'anonimo'
  ...
  return (
    <div>
      ...
      {posts.length === 0 && <p className="text-muted">{emptyMessage}</p>}

      <Row>
        {posts.map((post) => {
          const hasLocation = post.latitude != null && post.longitude != null
          const canDelete = post.username === currentUsername
            ...
            <Col key={post.id} xs={12} md={6} className="mb-4">
              <Card className="h-100 overflow-hidden">
                <PostHeader
                  ...
                  onDelete={canDelete ? () => setDeleteId(post.id) : undefined}
                />
